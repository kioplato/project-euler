#include <stdio.h>

int GetFile(FILE *fp, char line[], int max)     //diavazei olo to arxeio mexri na teliwsei o pinakas line i na ftasei sto EOF.
{
        int nch = 0;
        int c;
        max = max - 1;                  /* afinw xoro gia to '\0' */

        while((c = getc(fp)) != EOF)                            //oso den exoume ftasei sto EOF sinexise na diavazeis xaraktires.
                {
                        if(nch < max)
                        {
                                line[nch] = c;          //an den exei teliwsei o pikanas line anathese tin timi
                                nch++;
                        }
                }
        if(c == EOF && nch == 0)
                return EOF;

        line[nch] = '\0';
        return nch;                             //epestrepse posous xaraktires diavases.
}

int main()
{
        char line[] = "";
        unsigned long long maxAthrisma = 0;
        unsigned long long athrisma = 1;
        int counter = 0;
        int metritis = 0;
        while(line[counter] != '\0')
        {
                athrisma = 1;
                if(counter != 988)
                {

                        for(metritis = 0; metritis < 13; metritis++)
                        {
                                printf("%d\n", (int)(line[counter + metritis]));
                                athrisma = athrisma * ((int)line[counter + metritis] - 48);
                        }
                }
                if(counter == 988)
                {
                        break;
                }
                printf("%d\n", athrisma);
                system("pause");
                if(athrisma > maxAthrisma)
                {
                        maxAthrisma = athrisma;
                }
                counter++;
        }
        printf("Max: %Lui\n", maxAthrisma);
        return 0;
}

