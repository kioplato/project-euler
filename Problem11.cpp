...
int main(void)
{
	...
	while(i == 0)
	{
		plegma[GramesPlegma][StilesPlegma] = arxikoplegma[StilesArxikou];
		//printf("%d = ", plegma[GramesPlegma][StilesPlegma]);
		//printf("%d\n", arxikoplegma[StilesArxikou]);
		StilesArxikou++;
		if(StilesArxikou == 400)
		{
			break;
		}
		StilesPlegma++;
		...
		//printf("GramesPlegma: %d\n", GramesPlegma);
		//printf("StilesPlegma: %d\n", StilesPlegma);
		//printf("StilesArxikou: %d\n", StilesArxikou);
		
	}
//DOULEVEI MEXRI EDW !
	for(GramesPlegma = 0; GramesPlegma < 20; GramesPlegma++)
	{
		for(StilesPlegma = 0; StilesPlegma < 20; StilesPlegma++)
		{
			printf("%d ", plegma[GramesPlegma][StilesPlegma]);
		}
	}
